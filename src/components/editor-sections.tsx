...
export function ProfileEditor({ resume, onChange }: Pick<Props, "resume" | "onChange">) {
  ...
  const updateContact = (key: keyof Resume["contact"], value: string) => onChange({ ...resume, contact: { ...resume.contact, [key]: value } });
  const steps = [
    { title: "Identitas", done: resume.contact.fullName.trim() && resume.targetRole.trim(), view: <div className="field-grid"><Field label="Nama lengkap" value={resume.contact.fullName} onChange={(value) => updateContact("fullName", value)} /><Field label="Target role" value={resume.targetRole} onChange={(value) => onChange({ ...resume, targetRole: value })} /></div> },
    { title: "Kontak", done: resume.contact.email.trim() && resume.contact.location.trim(), view: <div className="field-grid"><Field label="Email" type="email" value={resume.contact.email} onChange={(value) => updateContact("email", value)} /><Field label="Telepon" value={resume.contact.phone} onChange={(value) => updateContact("phone", value)} /><Field label="Kota / negara" value={resume.contact.location} onChange={(value) => updateContact("location", value)} /></div> },
    { title: "Link", done: resume.contact.linkedin.trim() || resume.contact.portfolio.trim(), view: <div className="field-grid"><Field label="LinkedIn" value={resume.contact.linkedin} onChange={(value) => updateContact("linkedin", value)} /><Field label="Portfolio / GitHub" value={resume.contact.portfolio} onChange={(value) => updateContact("portfolio", value)} /></div> },
  ];
  return <FocusFlow steps={steps} active={step} onStep={setStep} />;
}
...
function updateExperience(resume: Resume, onChange: Props["onChange"], id: string, patch: Partial<Resume["experiences"][number]>) { onChange({ ...resume, experiences: resume.experiences.map((item) => item.id === id ? { ...item, ...patch } : item) }); }
function updateProject(resume: Resume, onChange: Props["onChange"], id: string, patch: Partial<Resume["projects"][number]>) { onChange({ ...resume, projects: resume.projects.map((item) => item.id === id ? { ...item, ...patch } : item) }); }
function EntryHeader({ title, action, onAdd }: { title: string; action: string; onAdd: () => void }) { return <div className="section-row"><h3>{title}</h3><button type="button" className="secondary-button compact" onClick={onAdd}><Plus size={16} />{action}</button></div>; }
function IconButton({ label, onClick }: { label: string; onClick: () => void }) { return <button type="button" className="icon-button danger" aria-label={label} title={label} onClick={onClick}><Trash2 size={17} /></button>; }
function FocusFlow({ steps, active, onStep }: { steps: Array<{ title: string; done: unknown; view: React.ReactNode }>; active: number; onStep: (next: number) => void }) {
  return <div className="focus-flow">
    <div className="focus-tabs" aria-label="Bagian formulir">{steps.map((item, index) => <button key={item.title} type="button" className={index === active ? "active" : ""} onClick={() => onStep(index)}><span>{item.done ? <CheckCircle2 size={14} /> : index + 1}</span>{item.title}</button>)}</div>
    <div className="focus-card" key={active}>{steps[active].view}</div>
    <div className="focus-actions">
      <button type="button" className="secondary-button" disabled={active === 0} onClick={() => onStep(Math.max(0, active - 1))}><ArrowLeft size={16} />Kembali</button>
      <button type="button" className="primary-button" disabled={active === steps.length - 1} onClick={() => onStep(Math.min(steps.length - 1, active + 1))}>Lanjut<ArrowRight size={16} /></button>
    </div>
  </div>;
}
export function Field({ label, value, onChange, type = "text", placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string }) { const id = `field-${label.toLowerCase().replace(/\W/g, "-")}`; return <label className="field" htmlFor={id}><span>{label}</span><input id={id} type={type} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} /></label>; }
export function TextArea({ label, value, onChange, rows, helper }: { label: string; value: string; onChange: (value: string) => void; rows: number; helper?: string }) { const id = `field-${label.toLowerCase().replace(/\W/g, "-")}`; return <label className="field" htmlFor={id}><span>{label}</span>{helper ? <small>{helper}</small> : null}<textarea id={id} value={value} rows={rows} onChange={(event) => onChange(event.target.value)} /></label>; }
